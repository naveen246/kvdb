...
func Test_RaftCommands(t *testing.T) {
	...
	assert.Equal(t, raft.Server{Suffrage: raft.Voter, ID: "node1", Address: "127.0.0.1:0"}, servers[0])
	assert.Equal(t, raft.Server{Suffrage: raft.Voter, ID: "node2", Address: "127.0.0.1:1"}, servers[1])
	...
	assert.Equal(t, 2, len(servers))
	assert.Equal(t, raft.Server{Suffrage: raft.Voter, ID: "node1", Address: "127.0.0.1:0"}, servers[0])
	assert.Equal(t, raft.Server{Suffrage: raft.Voter, ID: "node2", Address: "127.0.0.1:2"}, servers[1])

	leader := s.Leader()
	assert.Equal(t, "node1", string(leader.ID))
